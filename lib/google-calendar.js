...
/**
 * Belegte Zeiten aus allen konfigurierten Kalendern.
 *
 * Schlägt ein einzelner Kalender fehl (gelöscht, Zugriff entzogen), laufen die
 * übrigen weiter – sonst fiele die ganze Belegung weg. Die fehlgeschlagenen
 * Kalender hängen als `failures` am Ergebnis, damit ein Aufrufer sie melden
 * kann. Schlägt JEDER Kalender fehl, gibt es keine verwertbare Belegung mehr;
 * dann wird geworfen, statt ein leeres Ergebnis vorzutäuschen – ein leeres
 * Ergebnis hieße „alles frei“ und führte zu Doppelbuchungen.
 */
async function fetchBusyIntervals(refreshToken, timeMin, timeMax) {
  ...
  for (const calendarId of ids) {
    ...
    try {
      ...
      failures.push({ calendarId, message: err.message });
      continue;
    }
  ...
  if (failures.length === ids.length) {
    const err = new Error(failures.map((f) => `${f.calendarId}: ${f.message}`).join(' | '));
    err.calendarFailures = failures;
    throw err;
  }

  Object.defineProperty(alle, 'failures', { value: failures, enumerable: false });
  return alle;
}
