...
function findRentedCopyKeys(values) {
  if (!values || typeof values !== 'object') return [];
  return Object.entries(values)
    .filter(([, val]) => containsRentedCopy(val))
    .map(([key]) => key);
}
...
async function saveGroupOverrides(dbGet, dbRun, groupId, lang, values) {
  const blocked = findRentedCopyKeys(values);
  if (blocked.length) {
    const err = new Error(
      'Texte mit Miet-/Vermiet-Formulierungen können nicht gespeichert werden. Bitte neutrale Formulierungen verwenden.'
    );
    err.code = 'RENTED_COPY';
    err.fields = blocked;
    throw err;
  }
