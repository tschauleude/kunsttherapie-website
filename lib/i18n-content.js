...
const { containsRentedCopy } = require('./i18n-scrub');
...
const CATALOG = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'kt.methods.title', label: 'Methoden – Überschrift' },
      { key: 'kt.methods.sub', label: 'Methoden – Untertitel' },
      { key: 'kt.methods.1.title', label: 'Methoden Bild 1 – Titel' },
      { key: 'kt.methods.1.text', label: 'Methoden Bild 1 – Text' },
      { key: 'kt.methods.2.title', label: 'Methoden Bild 2 – Titel' },
      { key: 'kt.methods.2.text', label: 'Methoden Bild 2 – Text' },
      { key: 'kt.methods.3.title', label: 'Methoden Bild 3 – Titel' },
      { key: 'kt.methods.3.text', label: 'Methoden Bild 3 – Text' },
      { key: 'kt.methods.4.title', label: 'Methoden Bild 4 – Titel' },
      { key: 'kt.methods.4.text', label: 'Methoden Bild 4 – Text' },
...
function loadDefaultMessages() {
  const pages = i18nPersist.loadPagesExtra();
  return {
    de: { ...(pages.de || {}) },
    en: { ...(pages.en || {}) },
  };
...
const ABOUT_PAGE_HEADING = /^(über\s*mich|about\s*me)$/i;

/** Entfernt veraltete „Über mich“-Doppelüberschriften in about.name / about.kicker. */
function scrubAboutHeadingOverrides(overrides) {
  ...
  for (const lang of ['de', 'en']) {
    for (const [key, val] of Object.entries(overrides[lang] || {})) {
      ...
      if ((key === 'about.name' || key === 'about.kicker') && ABOUT_PAGE_HEADING.test(val.trim())) {
        changed = true;
        continue;
      }
...
function scrubOverrides(overrides) {
  const rented = scrubRentedOverrides(overrides);
  const about = scrubAboutHeadingOverrides(rented.overrides);
  return { overrides: about.overrides, changed: rented.changed || about.changed };
}
...
function parseOverrides(raw) {
  ...
  try {
    ...
    return scrubOverrides(base).overrides;
  } catch {
...
async function readOverrides(dbGet) {
  const fromFile = i18nPersist.readOverridesFile();
  let fromDb = emptyOverrides();
  try {
    const row = await dbGet(`SELECT value FROM settings WHERE key = ?`, [SETTINGS_KEY]);
    fromDb = parseOverrides(row?.value);
  } catch {
    /* settings-Tabelle noch nicht bereit */
  }
  return i18nPersist.mergeOverrideLayers(scrubOverrides(fromFile).overrides, fromDb);
}

async function seedOverridesFromFile(dbGet, dbRun) {
  const fromFile = scrubOverrides(i18nPersist.readOverridesFile()).overrides;
  const hasFile =
    Object.keys(fromFile.de).length > 0 || Object.keys(fromFile.en).length > 0;
  ...
  try {
    ...
    await writeOverrides(dbRun, fromFile);
    return true;
  }

  const dbEmpty = !Object.keys(fromDb.de).length && !Object.keys(fromDb.en).length;
...
async function syncI18nFromSource(dbGet) {
  const overrides = await readOverrides(dbGet);
  i18nPersist.rebuildSplitI18nFiles(overrides);
...
async function migrateRentedOverrides(dbGet, dbRun) {
  ...
  const fileOverrides = scrubOverrides(i18nPersist.readOverridesFile());
  if (fileOverrides.changed) {
    i18nPersist.writeOverridesFile(fileOverrides.overrides);
    changed = true;
  }
  ...
  if (row?.value) {
    ...
    if (parsed) {
      ...
      const cleaned = scrubOverrides(current);
      if (cleaned.changed) {
        await writeOverrides(dbRun, cleaned.overrides);
        changed = true;
      }
    }
  }
...
async function writeOverrides(dbRun, overrides) {
  const scrubbed = scrubOverrides(overrides).overrides;
  ...
  try {
    i18nPersist.persistOverrides(clean);
    invalidateDefaultsCache();
  } catch (e) {
    console.error('i18n persist (Datei/Code):', e.message);
  }
...
function validateGroupPayload(groupId, lang, values) {
  ...
  for (const key of Object.keys(values)) {
    ...
    if (containsRentedCopy(values[key])) {
      return `Text für „${key}“ enthält Miet-/Vermiet-Formulierungen – bitte ohne „angemietet“, „gemietet“ oder „rented“ formulieren.`;
    }
  }
  return null;
}
...
module.exports = {
  ...
  syncI18nFromSource,
  seedOverridesFromFile,
  scrubRentedOverrides,
  scrubAboutHeadingOverrides,
  scrubOverrides,
  invalidateDefaultsCache,
};
