...
const CATALOG = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'kt.audience.growing', label: 'Zielgruppen – Hinweis „wachsend“' },
      ...
      { key: 'kt.praxis.p2', label: 'Atelier – Beschreibung', html: true },
...
/** DB hat Texte, JSON-Datei leer → Datei für Deploy-Sicherheit füllen. */
async function syncOverridesToFile(dbGet) {
  const fromFile = scrubOverrides(i18nPersist.readOverridesFile()).overrides;
  const fileEmpty =
    !Object.keys(fromFile.de).length && !Object.keys(fromFile.en).length;
  ...
  i18nPersist.persistOverrides(overrides);
  return true;
}
