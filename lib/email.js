...
// ── Buchung: E-Mail-Verifizierung (Schritt 1) ─────────────────────────────────
// whenList: array of formatted date strings (one per selected slot)

async function sendBookingVerification({ name, email: toEmail, verifyUrl, whenList }) {
  if (!isEmailConfigured()) return { sent: false, reason: 'smtp_not_configured' };

  const from = process.env.SMTP_FROM;
  const transport = createTransport();
  const slots = Array.isArray(whenList) ? whenList : [whenList];
  const slotsHtml = slots.map((w) => `<p>${esc(w)}</p>`).join('');
  const plural = slots.length > 1 ? `e (${slots.length})` : '';

  const html = emailWrapper(`
    <p>Hallo ${esc(name)},</p>
    <p>bitte bestätigen Sie Ihre E-Mail-Adresse, um Ihre Terminanfrage abzuschicken.</p>
    <div class="info-box">
      <p><strong>Gewünschter${plural} Termin${plural}</strong></p>
      ${slotsHtml}
    ...
    <p class="note">Der Link ist 24 Stunden gültig. Nach der Bestätigung erhalten Sie eine Eingangsbestätigung.<br>Falls Sie kein Formular ausgefüllt haben, können Sie diese E-Mail ignorieren.</p>
  `);
  ...
  try {
    await transport.sendMail({
      from,
      to: toEmail,
      subject: `Terminanfrage bestätigen${slots.length > 1 ? ` (${slots.length} Termine)` : ` – ${slots[0]}`}`,
      text: `Hallo ${name},\n\nbitte bestätigen Sie Ihre E-Mail-Adresse:\n${verifyUrl}\n\nTermine:\n${slots.join('\n')}\n\nDer Link ist 24 Stunden gültig.\n\nKunsttherapie Paderborn`,
      html,
    });
    return { sent: true };
  } catch (e) {
    console.error('sendBookingVerification error:', e.message);
    return { sent: false, error: e.message };
  }
}

// ── Buchung: Anfrage eingegangen ──────────────────────────────────────────────
// bookingOrBookings: single booking object OR array of booking objects

async function sendBookingRequestEmails(bookingOrBookings) {
  if (!isEmailConfigured()) return { sent: false, reason: 'smtp_not_configured' };

  const bookings = Array.isArray(bookingOrBookings) ? bookingOrBookings : [bookingOrBookings];
  if (bookings.length === 0) return { sent: false, reason: 'no_bookings' };
  ...
  const whenList = bookings.map((b) => formatGermanDateTime(b));
  const slotsHtml = whenList
    .map((w) => `<p>${esc(w)}</p>`)
    .join('');
  const slotsText = whenList.join('\n');
  const subjectSuffix = bookings.length === 1 ? `– ${whenList[0]}` : `– ${bookings.length} Termine`;

  const clientHtml = emailWrapper(`
    <p>Hallo ${esc(first.name)},</p>
    <p>vielen Dank für Ihre Terminanfrage. Ich habe sie erhalten und melde mich zeitnah zur Bestätigung.</p>
    <div class="info-box">
      <p><strong>Gewünschter${bookings.length > 1 ? 'e Termine' : ' Termin'}</strong></p>
      ${slotsHtml}
      <p>Otto-Stadler-Straße 23c, 33102 Paderborn</p>
    </div>
    ${first.message ? `<p><em>Ihre Nachricht:</em> ${esc(first.message)}</p>` : ''}
    <hr class="divider">
  ...
  const practiceHtml = emailWrapper(`
    <p><strong>Neue Terminanfrage${bookings.length > 1 ? ` (${bookings.length} Termine)` : ` #${esc(String(first.id))}`}</strong></p>
    <div class="info-box">
      ${slotsHtml}
    </div>
    <div class="meta">
      <strong>${esc(first.name)}</strong><br>
      <a href="mailto:${esc(first.email)}">${esc(first.email)}</a><br>
      ${first.phone ? `${esc(first.phone)}<br>` : ''}
    </div>
    ${first.message ? `<div class="msg-box">${esc(first.message).replace(/\n/g, '<br>')}</div>` : ''}
  ...
  const [clientSent, practiceSent] = await sendIndependently(transport, [
    ...
    },
    {
      ...
      options: {
        ...
        to: practiceTo,
        replyTo: first.email,
        subject: `Neue Terminanfrage: ${first.name} ${subjectSuffix}`,
