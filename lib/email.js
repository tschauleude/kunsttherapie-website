  ...
  const [practiceSent, clientSent] = await sendIndependently(transport, [
    {
      ...
      options: {
        ...
        html: `<p><strong>Neue Anmeldung für: ${eventTitle}</strong>${when ? `<br>${when}` : ''}</p>
          <p><strong>${name}</strong> &lt;<a href="mailto:${fromEmail}">${fromEmail}</a>&gt;</p>
          ${phone ? `<p>Telefon: ${phone}</p>` : ''}
    ...
    },
    {
      ...
      options: {
        ...
        html: `<p>Hallo ${name},</p>
          <p>vielen Dank für die Anmeldung zu <strong>${eventTitle}</strong>${when ? ' am ' + when : ''}.</p>
