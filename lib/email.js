...
function buildClientEmailHtml(booking, links) {
  ...
  return `
    ...
    <p>dein Termin ist <strong>bestätigt</strong>:</p>
...
function buildClientRequestEmailHtml(booking) {
  ...
  return `
    <p>Hallo ${booking.name},</p>
    <p>vielen Dank für deine Terminanfrage:</p>
    <p><strong>${when}</strong><br>
    Otto-Stadler-Straße 23c, 33102 Paderborn</p>
    <p>Martina prüft die Anfrage und sendet dir die <strong>Bestätigung per E-Mail</strong>, sobald der Termin fest steht.</p>
...
async function sendBookingRequestEmails(booking) {
  ...
  await transport.sendMail({
    ...
    text: `Hallo ${booking.name},\n\nvielen Dank für deine Terminanfrage: ${when}\nMartina bestätigt den Termin zeitnah per E-Mail.\n\nMartina Schwierzke`,
...
async function sendContactMessage({ name, email: fromEmail, phone, message }) {
  ...
  await transport.sendMail({
    from,
    to: fromEmail,
    subject: 'Deine Nachricht – Kunsttherapie Paderborn',
    text: `Hallo ${name},\n\nvielen Dank für deine Nachricht. Martina meldet sich zeitnah bei dir.\n\nHerzliche Grüße\nMartina Schwierzke`,
    html: `<p>Hallo ${name},</p><p>vielen Dank für deine Nachricht. Martina meldet sich zeitnah bei dir.</p><p>Herzliche Grüße<br>Martina Schwierzke</p>`,
  });
...
async function sendAtelierSubmissionNotice(submission, baseUrl) {
  ...
  if (!submission.is_anonymous && submission.submitter_email) {
    await transport.sendMail({
      ...
      subject: 'Dein Werk ist angekommen – Kunsttherapie Paderborn',
      text: `Hallo${submission.submitter_name ? ' ' + submission.submitter_name : ''},\n\nvielen Dank für deine Einsendung aus dem Mini-Atelier. Martina hat dein Werk erhalten und kann es vertraulich als Impuls nutzen.\n\nHerzliche Grüße\nMartina Schwierzke`,
      html: `<p>Hallo${submission.submitter_name ? ' ' + submission.submitter_name : ''},</p>
        <p>vielen Dank für deine Einsendung aus dem Mini-Atelier. Martina hat dein Werk erhalten und kann es vertraulich als Impuls nutzen.</p>
        <p>Herzliche Grüße<br>Martina Schwierzke</p>`,
    });
  }
