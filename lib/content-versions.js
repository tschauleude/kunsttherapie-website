...
async function restoreSiteImages(dbGet, dbRun, snapshot) {
  const { slots, galleryCount } = normalizeSiteImagesSnapshot(snapshot);
  ...
  for (const [slot, data] of Object.entries(slots)) {
    ...
    await dbRun(
      ...
      [slot, data.url, data.updatedAt || null]
    );
  }
